...
- **A concept can be shortlisted: `PUT /api/mixlab/runs/{id}/concepts/{conceptId}/shortlist`.** Body `{shortlisted: true|false}` sets a `shortlisted` flag on that concept in the run manifest → `204`, `404` for an unknown run or concept, `400` when the field is missing or not a boolean. Idempotent. This is the operator marking "I intend to make this cut", and it is deliberately **not** feedback: no `MixLabFeedbackEvent` is queued, so nothing about shortlisting reaches engine history. Manifests written before this field omit it and read as `false`.
- **The run archive index carries `shortlistedCount` and `playedCount`.** Both are derived from the run's manifest — starred concepts, and concepts whose feedback verdict is `played` or `played_modified` — and are refreshed on every shortlist and feedback write. Manifest and index are separate blobs with separate ETags, so the refresh happens inside the index's ETag retry loop and re-reads the manifest on each attempt: because a competing writer always writes its manifest before its index, the writer that loses the race still sees the winner's manifest and the two converge. The refresh is deliberately **best-effort** — the manifest write has already landed by then, so any index failure (a lost ETag race, a blob-service error, a transport fault) is logged and the counts left stale rather than failing a request that succeeded; only cancellation propagates. Counts are therefore a projection to be treated as a hint and repaired by the reindex below, never a source of truth. Entries written before these fields existed omit them and read as `0`.
- **`POST /api/mixlab/runs/reindex` recomputes those counts from the manifests** → `200 {runs: n}`, where `n` is the number of runs whose manifest was read. Idempotent, and it applies the numbers with a copy-on-write over whatever the index holds at write time rather than replacing the array, so a concurrent worker claim or complete cannot be clobbered. Runs whose manifest is missing keep their entry unchanged. This is the backfill for legacy entries and the repair for the drift windows that exist — a crash between a manifest write and its index write, or a count refresh that exhausted its retries. Manual only: the sweep is not a snapshot, so a write landing mid-sweep is overwritten until the next run. Run it once per environment after deploying.
...
- **Deleting a run drops its pending feedback events.** Events queued in `feedback/pending.json` for a deleted run could only ever be skipped as unmatchable by the engine; they are now removed as part of the delete. Nothing is written (so no ETag churn under a worker ack) when the run has nothing pending.

## v1.65

### Features

- **`trackPool` run flag.** `POST /api/mixlab/runs` accepts an optional `trackPool` flag — a JSON-serialised track-pool restriction pinned for the run ("Run this block" in mixlab-web). Validated as a non-empty JSON-object string of at most 16000 characters and persisted verbatim into the run manifest; the worker maps it to the engine's `--track-pool`, which owns full validation. Existing enqueue payloads are unaffected. Runs pinned this way carry a `/block` suffix on `FlagsSummary` in the run archive index, alongside the existing mode/risk/directions summary.
